...
//  }
  ...
  def loadPhenotypes(phenotypesPath: String,
                     ...
                     covarNames: Option[List[String]]): RDD[Phenotype] = {
    ...
    val covariateDF = if (covarPath.isDefined) {
      Option(
        sparkSession.read.format("csv")
          ...
          .option("delimiter", delimiter)
          .load(covarPath.get)
          .select(covarNames.get.head, covarNames.get.tail: _*)
          .coalesce(1)
      )
    } else {
      None
    ...
    val phenoCovarDF = if (covariateDF.isDefined) {
      phenotypesDF.join(covariateDF.get, Seq("IID"))
    } else{
      phenotypesDF
    }
